...
役割:
  ...
  2. 地図APIを app_client.py 経由で代理呼び出し（ロボットAPIへ直接、API-keyヘッダーで認証）
  3. 地図画像を「同一オリジン」で代理配信（canvas汚染の回避 & EXIF取得のため）
  4. 合体PNGにベースのEXIFを入れ直して返す（キャンバス書き出しでEXIFが消えるため）

前提:
  - 同じフォルダに app_client.py があること
  - .env に ROBOT_ID を設定すること（DIRECT_API_HOST, API_KEY は任意。未指定ならapp_client.pyの既定値を使う）
...
# app_client.py 経由でロボットAPIへ直接アクセスする（API-keyヘッダーで認証、暗号化なし）
#   api_get(robot_id, api_name)         -> GET  を実行し、レスポンス本文（JSON文字列）を返す
